...
const updateUserAvatar = asyncHandler(async (req, res) => {
  const avatarLocalAvatar = req.file?.path;
  ...
  if (!avatar.url) {
    throw new ApiError(401, "error while upload avatar");
  }
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  const coverImageLocalAvatar = req.file?.path;

  if (!coverImageLocalAvatar) {
    throw new ApiError(400, "Avatar file is missing");
  }
  const coverImage = await uploadOnCloudinary(coverImageLocalAvatar);
  if (!coverImage.url) {
    throw new ApiError(401, "error while upload coverImage");
  }
